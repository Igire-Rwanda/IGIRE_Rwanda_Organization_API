...
class TeamController {
  ...
  static async createTeamate(req, res) {
    ...
    const user = await TeamInfo.create(req.body, {profileImage: req.file.path});
  ...
  static async getallTeam(req, res) {
    const users = await TeamInfo.find().select("title profileImage Position description").exec();
    if (!users) {
